...
class OVB_iCal_Service
{
    ...
    public static function render_calendar_buttons($order_id)
    {
        ...
        printf(
            '<a href="%s" target="_blank" class="button ov-btn" download="booking-%d.ics">📥 Dodaj u kalendar</a> ',
            esc_url($ics_url),
            $order_id
        );
        printf(
            '<a href="%s" target="_blank" class="button ov-btn">📅 Add to Google Calendar</a>',
            esc_url($google_url)
        );
        echo '</p>';
    }
