...
// add_action('wp_ajax_ovb_admin_create_manual_order', 'ovb_create_manual_order_from_admin');
// function ovb_create_manual_order_from_admin() {
//     if (!current_user_can('edit_products')) {
//         wp_send_json_error(__('Unauthorized access', 'ov-booking'));
//         wp_die();
//     }
...
//         $order->update_meta_data('_ovb_booking_id', sanitize_text_field($client_data['bookingId'] ?? ''));
//         $order->update_meta_data('_ovb_range_start', sanitize_text_field($client_data['rangeStart'] ?? ''));
//         $order->update_meta_data('_ovb_range_end', sanitize_text_field($client_data['rangeEnd'] ?? ''));
//         $order->update_meta_data('_ovb_guests', absint($client_data['guests'] ?? 1));

//         $order->set_status('completed');
//         $order->save();

//         ovb_log_error('Manual order created: ' . $order->get_id(), 'admin-calendar');
