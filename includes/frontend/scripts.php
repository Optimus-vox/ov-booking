    ...
    if (is_checkout() && !is_order_received_page()) {
        ...
        jQuery(document).ready(function($) {
            ...
            $(document.body).on('updated_checkout', function() {
                setTimeout(function() {
                    // Trigguruj samo ako nisu već mount-ovani
                    if (!stripeElementsMounted) {
                        $(document.body).trigger('wc_stripe_init_elements');
                        stripeElementsMounted = true;
                    }
                    
                    if (!klarnaInitialized && typeof window.KlarnaPayments !== 'undefined') {
                        $(document.body).trigger('init_klarna_payments');
