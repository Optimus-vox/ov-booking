...
foreach ($all_dates as $i => $date) {
    ...
    if ($i === count($all_dates) - 1) {
        $dates_output .= '<tr class="ovb-checkout-row"><td class="ovb-date-label">' . esc_html($pretty_date) . ':</td><td class="ovb-date-value checkout">' . esc_html__('Checkout', 'ov-booking') . '</td></tr>';
    } else {
